 ...
 * and how to resolve it, ie find the IP address where this service is
 * available. This class also shows how to keep informed of service updates.
 *
 * @author gilles
 *
 */
public class TestServiceBrowser implements IClientCallback,
        IServiceBrowserCallback,
        IServiceResolverCallback, Runnable {
     ...
     */
    @Override
    public synchronized void resolverCallback(ServiceResolver resolver,
            int interfaceNum,
    ...
    public void run() {
        ...
        TestServiceBrowser b;
        try {
            b = new TestServiceBrowser();

            while (!Thread.currentThread().isInterrupted()) {
                try {
                    b.browse();
                    System.out.println(
                            "Passando pela Thread do TestServiceBrowser");
                    System.out.println(
                            "Chegando no SLEEP");
                    Thread.sleep(5000);
                } catch (InterruptedException ex) {
                    Logger.getLogger(TestServiceBrowser.class.getName())
                            .log(Level.SEVERE, null, ex);
                }

            }
            b.stop();
        } catch (Avahi4JException ex) {
            Logger.getLogger(TestServiceBrowser.class.getName())
                    .log(Level.SEVERE, null, ex);
        }
    }
